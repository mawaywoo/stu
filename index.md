...
596. i) 模范畴中的某种结构建立的 **bijection** 事实上就是对模范畴的某种结构进行**分类**。
     ...
     iii) 如模范畴的挠对与环的某些理想或环的 Gabriel 拓扑以及模范畴的局部化等的对应就是对模范畴的挠结构进行分类。

     iv) 或者模范畴的某些特殊子范畴的分类，如 Serre 子范畴，definable 子范畴的分类等等。

     v) 类似于子拓扑斯和 Grothendieck 拓扑的**对应**，本质也是借助 Grothendieck 拓扑对子拓扑斯进行**分类**。
