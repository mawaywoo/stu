...
1131. 同样的构件方法，在不同的范畴中构建出来的结构是不一样的，因为不同范畴所提供的**基本构件是不一样的**，比如范畴 C 中的单纯对象构建，都是 **Delta 这种构造方式，选不同的底范畴 C，基本构件 X([n]) 是不一样的**（在 C **为 Cat 时，X([n]) 就已经含有对象和态射这些信息了**，而 C 若仅为某个 1-范畴时，X([n]) 仅表示 C 中的一个对象，再无其他信息），它们的粘接方式也会不一样，31:07；在高阶范畴中，通常会**把高阶态射前置地储藏在基本构件中**。在学习高阶范畴的各个结论时，要**尽量去“看”到高阶态射的位置和变换**。[BilTop Rune Haugseng | Higher Morita categories](https://www.youtube.com/watch?v=67gGDM0ouS8&t=257s) 2026-9-24 14:36 崇德苑 209 室
