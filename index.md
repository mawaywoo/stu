...
607. a) 生成的复形的同调群的位置揭示生成的复形范畴的大小。

     b) 某对象M同调群的情况（复形的同调群位置）决定与它相关的导出函子Ext(M, -)的复杂度。

     c) 同调导出函子/同调函子某种程度上也是在刻画某对象的复杂度，Ext^i(M, -)=0是对与M等价的投射分解复形的的长度进行刻画。

     d) 所以perpendicular范畴的对象也是对与其等价复形的长度进行某些限制。

     e) 各种同调维数其实本质上是衡量对象的复杂度，即该对象的“复形长度”。

     f) 各种同调维数=比较复形的“长短”。

     g) 同调=长度。

     h) dg-模上的各种条件往往也是复形上各个位置的同调群的限制。

     i) M是d- cluster tilting模是指使得由M按两种不同方式“生成”的模范畴addM和按导出函子Ext生成的模范畴相等的对象M。

     j) 一种是直和加直和项的生成方式，一种是同调的生成方式。2024-1-10 23:36
