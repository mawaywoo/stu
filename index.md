...
592. 左Kan扩张，**正向极限**，取对象x的**左邻域**（看正向极限的箭头）的所有对象（的原像）的**最大值**（即取正向极限）作为左Kan扩张在x的取值，右Kan扩张取对偶的情形。

593. **极限余极限简单想像为取最小值和最大值**，这在考虑函子对极限余极限的保持性及极限余极限的交换性时更加形象地理解，不至于弄混。

594. Sh_X = **X 上的正变函子范畴**，X 上的闭集和开集分别为下闭和上闭子集，这定义了 X 的拓扑，**关于拓扑的开集来说，层是反变函子**(见 Sect. 2.2 层的定义)。偏序集的 Hasse 图就是**偏序集的子部分**，就是**平时画出来的表示偏序集的图**。见 [On Derived Equivalences of Categories of Sheaves Over Finite Posets](https://arxiv.org/abs/math/0610685)
